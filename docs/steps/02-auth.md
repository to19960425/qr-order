...
- [x] `src/app/admin/login/page.tsx` を作成
  - メール + パスワード入力フォーム
...
- [x] shadcn/ui の Button, Input, Card コンポーネントを追加

### 2.4 管理画面レイアウト

- [x] `src/app/admin/layout.tsx` を作成
  - サイドバーナビゲーションを含むレイアウト
  - 認証チェックは Middleware に委譲（layout では不要）
- [x] `src/components/admin/Sidebar.tsx` を作成
  - ロゴ（☕ QR Order）
  - ナビリンク: 注文（📋）、メニュー（🍽）、席（🪑）
  - ログアウトボタン
  - 現在のパスに応じてアクティブ状態を表示
...
- [x] Supabase Dashboard の Authentication > Users からアカウントを作成

### 2.6 仮ページ作成

- [x] `/admin/orders/page.tsx` に仮の「注文ダッシュボード」テキストを配置
- [x] `/admin/menu/page.tsx` に仮の「メニュー管理」テキストを配置
- [x] `/admin/tables/page.tsx` に仮の「席管理」テキストを配置

### 2.7 動作確認

- [x] 未認証で `/admin/orders` にアクセス → `/admin/login` にリダイレクトされる
- [x] ログイン成功 → `/admin/orders` に遷移する
- [x] サイドバーのナビゲーションが機能する
- [x] ログアウト → `/admin/login` に戻る

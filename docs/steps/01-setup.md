...
- [x] `npx create-next-app@latest` で Next.js 15 プロジェクトを作成
  - TypeScript: Yes
  - ESLint: Yes
  - Tailwind CSS: Yes
  - App Router: Yes
- [x] 不要な初期ファイルを整理（デフォルトの page.tsx の内容をクリア等）

### 1.2 依存パッケージのインストール

- [x] Supabase 関連: `@supabase/supabase-js`, `@supabase/ssr`
- [x] UI: `shadcn/ui` の初期化（`npx shadcn@latest init`）
- [x] QRコード: `qrcode`, `@types/qrcode`
- [x] PDF: `jspdf`
- [x] テスト: `vitest`, `@testing-library/react`, `@testing-library/jest-dom`, `jsdom`
- [x] E2E: `playwright`（`npx playwright install`）
...
- [x] `tailwind.config.ts` にカスタムカラーを追加
  ...
  accent: '#8B6914'
  ```
- [x] `.env.local.example` を作成（`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `NEXT_PUBLIC_APP_URL`）
- [x] `.env.local` を作成し、Supabase Cloud の値を設定
- [x] `vitest.config.ts` を作成
- [x] `playwright.config.ts` を作成
- [x] `.gitignore` に `.env.local` を追加確認

### 1.4 Supabase プロジェクト作成

- [x] Supabase Dashboard で新規プロジェクトを作成
- [x] プロジェクトURL と anon key を `.env.local` に設定

### 1.5 Supabase クライアント設定

- [x] `src/lib/supabase/client.ts` を作成（`createBrowserClient()`）
- [x] `src/lib/supabase/server.ts` を作成（`createServerClient()`）
...
- [x] `supabase/migrations/001_initial.sql` を作成
  ...
  - `create_order` RPC 関数
  - RLS ポリシー（全テーブル）
- [x] Supabase Dashboard の SQL Editor で実行、またはCLIで `supabase db push`

### 1.7 Supabase Storage セットアップ

- [x] Supabase Dashboard で `menu-images` バケットを作成（パブリック）

### 1.8 シードデータ投入

- [x] `supabase/seed.sql` を作成
  - 店舗データ（1件）
...
- [x] `src/types/database.ts` を Supabase CLI で自動生成
  ```bash
...
- [x] Server Component から Supabase のデータ取得ができることを確認

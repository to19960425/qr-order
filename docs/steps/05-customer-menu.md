...
- [x] `src/app/order/[token]/page.tsx` を作成
...
- [x] `tables.is_active = false` の場合に表示するUI
...
- [x] `src/components/order/CategoryTabs.tsx` を作成
  - 横スクロール可能なタブUI
...
- [x] `src/components/order/MenuCard.tsx` を作成
  - 写真を大きく表示するカード型レイアウト
  - 表示項目: 画像、名前、説明（抜粋）、価格
  - カートに未追加時: [+] ボタンのみ
...
- [x] `src/components/order/FloatingCartBar.tsx` を作成
...
- [x] `/order/{valid_token}` でメニューが表示される
- [x] `/order/{invalid_token}` でエラー画面が表示される
- [x] クローズ中のテーブルで営業時間外画面が表示される
- [x] カテゴリタブで絞り込みが動作する
- [x] メニューカードの [+] ボタンでカートに追加される
- [x] フローティングカートバーに点数・金額が反映される
